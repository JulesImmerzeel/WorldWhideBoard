...
class canvas
{
    constructor (container, sendFunction)
    {
        ...
        this.canvas.addEventListener("touchmove", (e) => ((c) => {
            ...
            c.outgoingLines.push({prev:{x:parseInt(c.prev.x * scalingFactor), y:parseInt(c.prev.y * scalingFactor)}, curr:{x:parseInt(c.curr.x * scalingFactor), y:parseInt(c.curr.y * scalingFactor)}, color:c.color, lineWidth: c.lineWidth});
            c.prev.x = c.curr.x;
            c.prev.y = c.curr.y;
            c.draw();            
        })(this), false);
